...
function sortBreweriesAlphabeticallyByName(breweryList) {
    return breweryList.sort((a, b) => (a.name > b.name) ? 1 : -1)
}
...
function getBreweriesBySelectedCities() {
    state = []
    let tempState = []
    selectedCities.forEach( function (thisCity) {
        currentSearchCriteria.by_city = thisCity
        ...
        fetch(BREWERIES_BASE_URL + CURRENT_SEARCH_CRITERIA)
            ...
            .then((breweries) => {
                ...
                if (breweries.length === 0) {
                    ...
                    tempState = sortBreweriesAlphabeticallyByName([...state, ...onlyIncludeBreweryTypes(breweries)])
