...
const searchButton = document.querySelector(
  "#select-state-form > input[type=submit]:nth-child(3)"
);
...
function stateAndTypeAPIs() {
  ...
  let searchUrl = root + `?by_state=${stateValue}`;

  // If statement to ensure that all breweries come up with the type Mirco, Regional and Brewpub when a state is entered:
  if (brewTypeValue) {
    // Get the root, followed by the state and type of brewery API.
    searchUrl = root + `?by_state=${stateValue}&by_type=${brewTypeValue}`;
  }
  // API call and render
  usState(searchUrl);
}
